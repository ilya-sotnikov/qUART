    ...
    QDialog{parent}
{
    ...
    for (const auto& info : portsInfo) {
        QString infoStr{"Port: " + info.portName() + "\n"
                        ...
                        + "Vendor identifier: " + (info.hasVendorIdentifier() ? QString::number(info.vendorIdentifier(), 16) : "") + "\n"
                        + "Product identifier: " + (info.hasProductIdentifier() ? QString::number(info.productIdentifier(), 16) : "") + "\n"};
