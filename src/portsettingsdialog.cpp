...
PortSettingsDialog::PortSettingsDialog(QWidget *parent)
    : QDialog{parent}, portsInfoDialog{new PortsInfoDialog{this}},
      serialPortBox{new QComboBox{this}}, baudRateBox{new QComboBox{this}},
      dataBitsBox{new QComboBox{this}}, parityBox{new QComboBox{this}},
      stopBitsBox{new QComboBox{this}}, flowControlBox{new QComboBox{this}} {
  auto layout{new QVBoxLayout{this}};
  auto buttonBox{new QDialogButtonBox{
      QDialogButtonBox::Cancel | QDialogButtonBox::Ok, Qt::Horizontal, this}};
  ...
  connect(portsInfoButton, &QPushButton::pressed, portsInfoDialog,
          &PortsInfoDialog::show);
  connect(buttonBox, &QDialogButtonBox::accepted, this,
          &PortSettingsDialog::ok);
  connect(buttonBox, &QDialogButtonBox::rejected, this,
          &PortSettingsDialog::cancel);
...
void PortSettingsDialog::fillSettings() const {
  ...
  flowControlBox->addItem(QStringLiteral("RTS/CTS"),
                          QSerialPort::HardwareControl);
  flowControlBox->addItem(QStringLiteral("XON/XOFF"),
...
void PortSettingsDialog::updateSettings() {
  ...
  currentSettings.baudRate =
      baudRateBox->currentData().value<QSerialPort::BaudRate>();
  currentSettings.dataBits =
      dataBitsBox->currentData().value<QSerialPort::DataBits>();
  currentSettings.parity =
      parityBox->currentData().value<QSerialPort::Parity>();
  currentSettings.stopBits =
      stopBitsBox->currentData().value<QSerialPort::StopBits>();
  currentSettings.flowControl =
      flowControlBox->currentData().value<QSerialPort::FlowControl>();
}
 ...
 *
 */
void PortSettingsDialog::cancel() { hide(); }
